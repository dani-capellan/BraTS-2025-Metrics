...
def get_LesionWiseScores(prediction_seg, gt_seg, label_value, dil_factor):
    ...
    # Get HD95 and NSD scores for the full image
    ...
    if np.all(gt_mat == 0) and np.all(pred_mat == 0):
        ...
        full_nsd = surface_distance.compute_surface_dice_at_tolerance(full_sd, 2)
    ...
    for gtcomp in range(np.max(gt_label_cc)):
        ...
        nsd = surface_distance.compute_surface_dice_at_tolerance(surface_distances, 2)

        metric_pairs.append((intersecting_cc,
                            gtcomp, gt_vol, dice_score, hd, nsd))
    ...
    return tp, fn, fp, gt_tp, metric_pairs, full_dice, full_hd95, full_gt_vol, full_pred_vol, full_sens, full_specs, full_nsd
...
def get_LesionWiseResults(pred_file, gt_file, challenge_name, output=None):
    ...
    for l in range(len(label_values)):
        ...
        tp, fn, fp, gt_tp, metric_pairs, full_dice, full_hd95, full_gt_vol, full_pred_vol, full_sens, full_specs, full_nsd = get_LesionWiseScores(
        ...
        metric_df = pd.DataFrame(
            metric_pairs, columns=['predicted_lesion_numbers', 'gt_lesion_numbers',
                                   'gt_lesion_vol', 'dice_lesionwise', 'hd95_lesionwise', 'nsd_lesionwise']
        ...
        try:
            lesion_wise_nsd = np.sum(
                metric_df_thresh['nsd_lesionwise'])/(len(metric_df_thresh) + len(fp))
        except:
            lesion_wise_nsd = np.nan
        ...
        metrics_dict = {
            ...
            'LesionWise_Score_NSD': lesion_wise_nsd,
        }
